...
WHERE class_cID = "59";                     -- depends on what class they are trying to access

-- #2
-- view classes registered by student with their ID number 593
SELECT * 
FROM classRoster
WHERE cl_id ="843";                         -- Depends on the person logged in
...
WHERE s_ID = "843"                          --  depends on who is logged in
    AND s_ID = cl_ID
...
SELECT * 
FROM notePages
WHERE n_cID = "25";                -- depends on which class's notes they are looking for
...
WHERE n_cID = cl_cID 
    AND cl_ID = s_ID 
    AND s_ID = "843";                               -- depends on who is logged in
...
WHERE p_ID = cla_ID 
    AND p_ID = "380"                         -- Depends on the class that is selected
    AND cla_cID = cl_cID;
...
DELETE 
FROM notePages 
WHERE n_nID = "7";  -- Depends on the specific file that would be deleted

-- #11
-- returns all of the classes a professor is teaching
SELECT *
FROM professor
WHERE p_ID = "710";   -- Depends on which professor is logged in

-- #12
-- Add new student to class roster for CSE 120
INSERT INTO classRoster (cl_name, cl_ID,cl_cID)
SELECT s_name, s_ID , cla_cID
FROM student, classCatalog
WHERE s_ID = "589"
    AND cla_cID = "59";   -- Depending on inputs from user, this would determine who is inputted to where
...
from classFolder
where class_cID = "35";   -- Depending on what is selected that class would be selected
...
WHERE a_ID = "780"    -- would depend on who is logged in

-- #20
-- Show all students in the Database who are in their 4th year AND taking at least one Math Classes
SELECT DISTINCT s_name, s_ID
